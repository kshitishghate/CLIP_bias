...
model_log_path = os.path.join('results','logs','cfd_7b_scseat.log')
...
def load_results():
    ...
    results_path = os.path.join('results','data','cfd_7b_scseat.csv')
...
def fit_regression(image_set, target, predictors, interactions=None, cubic_terms=None, threeway_interactions=None,
    ...
    if cubic_terms is not None:
        formula += ' + ' + ' + '.join([f'I({a}**3)' for a in cubic_terms])
...
full_model, full_diag, full_lmbda, full_data = fit_regression(
    ...
    use_boxcox=False,
    interactions=[('GenderSelf', 'EthnicitySelf')],
    threeway_interactions=None,
    cov_type='HC3',
...
genders['dif'] = genders['predictions_x'] - genders['predictions_y']
...
sns.regplot(x=data['AgeRated'], y=best_model.resid,lowess=True, line_kws={'color':'red'})
