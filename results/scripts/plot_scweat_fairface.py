...
def load_results(largest_model_only=True, only_emotions=True, exclude_negations=True,
                 templates_to_filter_to=None, dataset='cfd'):
    dataset = '' if dataset == 'cfd' else '_' + dataset

    results_path = os.path.join('results','data',f'wna_scweat{dataset}.csv')
    ...
    results['w_len'] = results['w'].str.len()
    return results

def fit_regression(B, variables, dataset, use_boxcox = True):
    all_results = load_results(largest_model_only=False, dataset=dataset)
    ...
    mf = pd.concat([mf] + [pd.get_dummies(mf[v], prefix=v, drop_first=True) for v in variables if v != 'w_len'],axis=1)
...
res0, diag0, lmbda0 = fit_regression(B='Asian-American',variables=['model','template_base'],dataset='fairface')
res1, diag1, lmbda1 = fit_regression(B='Asian-American',variables=['model','template_base','emotion_type'],dataset='fairface')
res2, diag2, lmbda2 = fit_regression(B='Asian-American',variables=['model','template_base','emotion_type','emotion','w_len'],dataset='fairface')
res3, diag3, lmbda3 = fit_regression(B='Asian-American',variables=['model','template_base','emotion_type','emotion','word'],dataset='fairface')
diag2.residual_plot()
diag2.qq_plot()


res0c, diag0c, lmbda0c = fit_regression(B='Asian-American',variables=['model','template_base'],dataset='cfd')
res1c, diag1c, lmbda1c = fit_regression(B='Asian-American',variables=['model','template_base','emotion_type'],dataset='cfd')
res2c, diag2c, lmbda2c = fit_regression(B='Asian-American',variables=['model','template_base','emotion_type','emotion', 'w_len'],dataset='cfd')
res3c, diag3c, lmbda3c = fit_regression(B='Asian-American',variables=['model','template_base','emotion_type','emotion','w_len','word'],dataset='cfd')
diag2c.qq_plot()
