...
def plot_results():
    ...
    results['Context (Comparison Method)'] = results['context'] + ' (' + results['comparison'] + ')'
    ...
    order.sort()
    hue_order = results['Context (Comparison Method)'].unique()
    hue_order.sort()
    for emotion, data in results.groupby('emotion'):
        sns.catplot(y='X vs. Y',x='effect_size', data=data, order=order, aspect=1.5, hue='Context (Comparison Method)',
